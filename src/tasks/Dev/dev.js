...
//a collection of commands primarily useful for developers
  ...
  static version() {
    return {
      ...
      process: function (bot, msg, suffix) {
        let commit = require('child_process').spawn('git', ['log', '-n', '1']);
  ...
  static userid() {
    return {
      ...
      process: function (bot, msg, suffix) {
        if (suffix) {
          let users = msg.channel.guild.members.filter((member) => member.user.username == suffix).array();
          if (users.length == 1) {
